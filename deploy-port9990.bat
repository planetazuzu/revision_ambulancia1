@echo off
REM 🚀 Script de Despliegue para AmbuReview - Puerto 9990
...
if not exist .env (
    echo [WARNING] Archivo .env no encontrado. Creando desde env.port9990...
    copy env.port9990 .env
    echo [SUCCESS] Archivo .env creado para puerto 9990
) else (
...
echo [INFO] Construyendo y levantando servicios en puerto 9990...
...
echo.
echo [SUCCESS] 🎉 ¡Despliegue en puerto 9990 completado!
echo.
...
echo   Ver logs: docker compose -f docker-compose.port9990.yml logs -f [servicio]
echo   Parar servicios: docker compose -f docker-compose.port9990.yml down
echo   Reiniciar servicios: docker compose -f docker-compose.port9990.yml restart
echo   Acceder a base de datos: docker compose -f docker-compose.port9990.yml exec db psql -U app -d appdb
echo.
echo [SUCCESS] ¡Sistema AmbuReview listo en puerto 9990! 🚀
echo.
pause
